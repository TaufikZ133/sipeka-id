...
export default function ModalUploadIjazah(props) {
  ...
  const getFotoIjazah = async () => {
    try {
      const response = await axios.get(
        process.env.REACT_APP_API_URL+`/karyawan/${props.MyId}`
      );
      if (response.status === 200) {
        setFotoIjazah(process.env.REACT_APP_API_URL+"/"+ response.data.foto_ijazah);
      }
      if (response.data.foto_ijazah.length === 0) {
        setFotoIjazah("http://fakeimg.pl/400x600/?text=Ijazah");
        console.log("Foto Ijazah Tidak ada");
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleTemporaryImage = (e) => {
    ...
    setFotoIjazah(URL.createObjectURL(uploaded));
  };

  const uploadIjazah = async (e) => {
    e.preventDefault();
    try {
      if (temporaryImage !== null) {
        ...
        if (response.status === 200) {
          await axios.patch(
            ...
            {
              foto_ijazah: response.data.image,
            }
          );
          alert("Foto Ijazah Anda Berhasil di Update");
        }
      }
    } catch (error) {
      console.log(error);
    ...
    <div className="z-50 h-screen w-screen fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50 py-10">
      <div className="bg-white animate-fade rounded-xl shadow-lg">
        ...
        <div className="mt-1 mb-5 mx-auto p-4">
          <img src={fotoIjazah} className="h-[33rem] w-[26rem]" alt="..." />
          <p className="p-2">Upload Ijazah Disini</p>
