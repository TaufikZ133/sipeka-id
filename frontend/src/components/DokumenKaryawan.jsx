...
//Modal
import ModalSuratCuti from "../modal/ModalSuratCuti.jsx";
import ModalSuratLembur from "../modal/ModalSuratLembur.jsx";
import ModalSuratKerja from "../modal/ModalSuratKerja.jsx";

export default function DokumenKaryawan() {
  const navigate = useNavigate();
  const [open, setOpen] = useState(true);
  const [dataDokumen, setDataDokumen] = useState([]);
  const [query, setQuery] = useState("");
  const [myDetail, setMyDetail] = useState(null);
  const [showModalSuratCuti, setShowModalSuratCuti] = useState(false);
  const [showModalSuratLembur, setShowModalSuratLembur] = useState(false);
  const [showModalSuratKerja, setShowModalSuratKerja] = useState(false);
  const { id } = useParams();
  ...
  const handleCetakDokumen = async (id) => {
    if (id === 1) {
      setShowModalSuratCuti(true);
    }
    if (id === 2) {
      setShowModalSuratLembur(true);
    }
    if (id === 3) {
      setShowModalSuratKerja(true);
    }
    if (id > 3) {
      alert("Cetak PDF belum Tersedia");
    }
  };

  const handleCloseModalCuti = () => {
    setShowModalSuratCuti(false);
  };

  const handleCloseModalLembur = () => {
    setShowModalSuratLembur(false);
  };

  const handleCloseModalKerja = () => {
    setShowModalSuratKerja(false);
  };

  const getDokumen = async () => {
    try {
      const response = await axios.get(
        process.env.REACT_APP_API_URL + "/dokumen"
      );
      setDataDokumen(response.data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    ...
    <section className="flex">
                ...
                <div className="overflow-y-scroll w-full h-[25rem]">
                  <table className="w-full block md:table divide-x select-none">
                    <thead className="block md:table-header-group">
                      <tr className="md:border-none block md:table-row absolute md:top-auto md:left-auto md:relative h-[3rem]">
                        ...
                        <th className="bg-gray-100 text-black font-semibold text-[13px] md:border-none text-left block md:table-cell px-2">
                          Nama
                        </th>
                        <th className="bg-gray-100 text-black font-semibold text-[13px] md:border-none text-left block md:table-cell px-2">
                          Jenis Dokumen
                    ...
                    <tbody className="block md:table-row-group md:border-none text-[17px]">
                      {dataDokumen
                        .filter((dokumens) =>
                          dokumens.nama_dokumen.toLowerCase().includes(query)
                        )
                        .map((dokumens, index) => (
                          <tr
                            className="even:bg-white odd:bg-[#CBDBF4] h-[5rem]"
                            key={dokumens.id}
                          >
                            <td className="p-1 md:border-none text-left block md:table-cell pl-5">
                              {index + 1}
                            </td>
                            <td className="p-1 md:border-none text-left block md:table-cell pl-5">
                              {`DOC.SLK.000` + index}
                            </td>
                            <td className="p-1 md:border-none text-left block md:table-cell">
                            ...
                            <td className="p-1 md:border-none text-left block md:table-cell">
                              <button>
                                <svg
                                  ...
                                  xmlns="http://www.w3.org/2000/svg"
                                  onClick={() =>
                                    handleCetakDokumen(dokumens.id)
                                  }
                                >
      ...
      <div className="z-30">
        {showModalSuratCuti && (
          <ModalSuratCuti
            closeModalCuti={handleCloseModalCuti}
            data={myDetail}
          />
        )}
      </div>
      <div className="z-40">
        {showModalSuratLembur && (
          <ModalSuratLembur
            closeModalLembur={handleCloseModalLembur}
            data={myDetail}
          />
        )}
      </div>
      <div className="z-50">
        {showModalSuratKerja && (
          <ModalSuratKerja
            closeModalKerja={handleCloseModalKerja}
            data={myDetail}
          />
        )}
      </div>
