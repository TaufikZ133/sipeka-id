...
//Modal
import ModalTambahSlipGaji from "../modal-admin/ModalTambahSlipGaji.jsx";
import ModalLihatSlipGaji from "../modal-admin/ModalLihatSlipGaji.jsx";
import ModalLihatPresensi from "../modal-admin/ModalLihatPresensi.jsx";

export default function KelolaGaji() {
  const { id } = useParams();
  const SIDEBAR_WIDTH = 300;
  const navigate = useNavigate();
  const [open, setOpen] = useState(true);
  const [dataKaryawan, setDataKaryawan] = useState([]);
  const [modalTambahGaji, setModalTambahGaji] = useState(false);
  const [modalLihatGaji, setModalLihatGaji] = useState(false);
  const [modalLihatPresensi, setModalLihatPresensi] = useState(false);
  const [dataGajiKaryawan, setDataGajiKaryawan] = useState([]);
  const [dataPresensi, setDataPresensi] = useState([]);
  const [dataSatuKaryawan, setDataSatuKaryawan] = useState("");
  ...
  const getKaryawan = async () => {
    try {
      const response = await axios.get(
        process.env.REACT_APP_API_URL + "/karyawan"
      );
      setDataKaryawan(response.data.rows);
    } catch (error) {
      console.log(error);
    }
  };

  const handleTambah = async (id) => {
    const response = await axios.get(
      process.env.REACT_APP_API_URL + `/karyawan/${id}`
    );
    setDataSatuKaryawan(response.data);
    setModalTambahGaji(true);
    setMyCurrentId(id);
  };

  const handleDelete = async (id) => {
    try {
      await axios.delete(process.env.REACT_APP_API_URL + `/karyawan/${id}`);
      alert("Akun Karyawan Berhasil Di Hapus Dari Sistem");
      getKaryawan();
    } catch (error) {
      console.log(error);
    }
  ...
  const handleCloseTambahGaji = () => {
    setModalTambahGaji(false);
    setMyCurrentId(null);
  };

  const handleCloseLihatGaji = () => {
    setModalLihatGaji(false);
  };

  const handleCloseLihatPresensi = () => {
    setModalLihatPresensi(false);
  };
  ...
  const handlePeriksa = async (id) => {
    try {
      ...
      if (response.data.rows.length === 0) {
        alert("Karyawan Belum Memiliki Slip Gaji");
        setModalLihatGaji(false);
      }
      if (response.data.rows.length !== 0) {
        setDataGajiKaryawan(response.data.rows);
        setModalLihatGaji(true);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleLihatPresensi = async (id) => {
    try {
      const response = await axios.post(
      ...
      if (response.data.rows.length === 0) {
        alert("Karyawan Belum Memiliki Data Presensi");
        setModalLihatPresensi(false);
      }
      if (response.data.rows.length !== 0) {
        setDataPresensi(response.data.rows);
        setModalLihatPresensi(true);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <section className="flex h-fulll">
        ...
        <div>
          <ul className="m-auto">
            ...
            <div className="border-dashed border-opacity-50 border-2 border-gray-200 rounded-lg mx-1">
              ...
              <li onClick={() => navigate(`/master-gaji/${id}`)} className="cursor-pointer flex items-center gap-x-4 p-4 hover:opacity-50 rounded-md mt-2 mx-3 mb-1">
                <svg
      ...
      >
        ...
        <div className="content-admin h-full">
          <div className="w-full h-full bg-gray-300 pb-8">
            ...
            <div className="content-gaji bg-white h-[40rem] p-2 font-semibold rounded-[12px] mx-[1rem] mt-[2rem] shadow-2xl">
              ...
              <div className="overflow-y-scroll h-[30rem]">
                <table className="block md:table divide-x select-none w-full">
                  <thead className="block md:table-header-group">
                    <tr className="md:border-none block md:table-row absolute md:top-auto md:left-auto md:relative h-[4rem]">
                      <th className="bg-gray-100 rounded-tl-2xl text-black font-semibold text-[13px] md:border-none text-left block md:table-cell px-5">
                        No
                      </th>
                      ...
                      <th className="bg-gray-100 text-black font-semibold text-[13px] md:border-none text-left block md:table-cell pl-10">
                        Lihat Presensi
                      </th>
                      <th className="bg-gray-100 text-black font-semibold text-[13px] md:border-none text-left block md:table-cell pl-10">
                        Hapus Akun
                      </th>
                      <th className="bg-gray-100 text-black font-semibold text-[13px] md:border-none text-center block md:table-cell w-[10rem]">
                        Slip Gaji
                      </th>
                    </tr>
                  </thead>
                  <tbody className="block md:table-row-group md:border-none font-[420] text-[15px]">
                    {dataKaryawan
                      .filter((karyawan) =>
                        karyawan.nama.toLowerCase().includes(query) || karyawan.jabatan.toLowerCase().includes(query)
                      )
                      .map((karyawan, index) => (
                        <tr
                          ...
                          key={karyawan.id}
                        >
                          <td className="md:border-none text-left block md:table-cell px-5">
                            {index + 1}
                          </td>
                          <td className="py-5 md:border-none text-left block md:table-cell">
                            <img
                              alt="img"
                              className="rounded-xl h-[70px] min-w-[70px] bg-gray-200 text-[0px]"
                              onError={(e) =>
                                (e.target.src =
                                  "https://fakeimg.pl/100/?text=Foto")
                              }
                              src={
                          ...
                          <td className="md:border-none text-left bock md:table-cell">
                            {karyawan.nama}
                          </td>
                          <td className="md:border-none text-left block md:table-cell">
                            {karyawan.nik}
                          </td>
                          <td className="md:border-none text-left block md:table-cell">
                            {karyawan.email}
                          ...
                          <td className="md:border-none text-center block md:table-cell">
                            <button>
                              <svg
                                ...
                                stroke="currentColor"
                                className="w-6 h-6"
                                onClick={() => handleLihatPresensi(karyawan.id)}
                              >
                                <path
                                  strokeLinecap="round"
                          ...
                          <td className="py-6 md:border-none text-left block md:table-cell pl-10">
                            <button className="pr-5">
                              <svg
                                ...
                                stroke="currentColor"
                                className="w-7 h-7"
                                onClick={() => handleTambah(karyawan.id)}
                              >
                                <path
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                            ...
                            <button className="">
                              <svg
                                ...
                                stroke="currentColor"
                                className="w-6 h-6"
                                onClick={() => handlePeriksa(karyawan.id)}
                              >
                                <path
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
      ...
      <div className="modal-tambah-slipGaji">
        {modalTambahGaji && (
          <ModalTambahSlipGaji
            close={handleCloseTambahGaji}
            data={dataSatuKaryawan}
            myCurrentId={myCurrentId}
          />
        )}
      </div>
      <div className="modal-lihat-slipGaji">
        {modalLihatGaji && (
          <ModalLihatSlipGaji
            close={handleCloseLihatGaji}
            data={dataGajiKaryawan}
          />
        )}
      </div>
      <div className="modal-lihat-presensi">
        {modalLihatPresensi && (
          <ModalLihatPresensi
            close={handleCloseLihatPresensi}
            data={dataPresensi}
          />
        )}
      </div>
