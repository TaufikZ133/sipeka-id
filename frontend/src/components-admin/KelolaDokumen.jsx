...
//modal
import ModalTambahDokumen from "../modal-admin/ModalTambahDokumen.jsx";
...
export default function KelolaDokumen() {
  ...
  const [modalTambahDokumen, setModalTambahDokumen] = useState(false);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(process.env.REACT_APP_API_URL + `/dokumen/${id}`);
      alert("Dokumen Berhasil Di Hapus Dari Sistem");
      getDokumen();
    } catch (error) {
      console.log(error);
    }
  ...
  const handleCloseModal = () => {
    setModalTambahDokumen(false);
  };

  const TambahDokumen = () => {
    setModalTambahDokumen(true);
  };
  ...
  return (
    <section className="flex h-fulll">
                ...
                <table className="w-full block md:table divide-x select-none">
                  <thead className="block md:table-header-group">
                    <tr className="md:border-none block md:table-row absolute md:top-auto md:left-auto md:relative h-[4rem]">
                      ...
                      <th className="bg-gray-100 text-black font-semibold text-[13px] md:border-none text-left block md:table-cell pl-9">
                        Aksi
                      </th>
                    </tr>
                  </thead>
                  <tbody className="block md:table-row-group md:border-none font-[420] text-[15px]">
                    {dataDokumen
                      .filter((dokumen) =>
                        dokumen.nama_dokumen.toLowerCase().includes(query)
                      )
                      .map((dokumen, index) => (
                        <tr
                        ...
                        >
                          ...
                          <td className="px-3 md:border-none text-left block md:table-cell pr-5">
                            {`DOC.SLK.000` + index}
                          </td>
                          <td className="px-3 md:border-none text-left block md:table-cell pr-5">
                            {dokumen.nama_dokumen}
                          </td>
                          <td className="px-3 md:border-none text-left block md:table-cell pl-5">
                            {dokumen.jenis_dokumen}
                          </td>
                          <td className="py-6 md:border-none text-left block md:table-cell pl-5">
      ...
      <div className="modal-tambah-dokumen">
        {modalTambahDokumen && <ModalTambahDokumen close={handleCloseModal} />}
      </div>
    </section>
  );
}
